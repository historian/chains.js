...
exports.http = dispatcher({
  'method': function(stack, method) {
    return (this.kernel)(stack.push(function(ctx, clb){
      if (ctx.method == method) { clb(ctx); }
      else { clb.pass(ctx); }
    }));
  },
  'path': function(stack, path) {
    return (this.kernel)(stack.push(function(ctx, clb){
      if (ctx.path == path) { clb(ctx); }
      else { clb.pass(ctx); }
    }));
  },
  'host': function(stack, host) {
    return (this.kernel)(stack.push(function(ctx, clb){
      if (ctx.host == host) { clb(ctx); }
      else { clb.pass(ctx); }
    }));
  },
...
try { var $ = require('jquery'); } catch(e) {}
if ($) { // <!--[jquery]-->
